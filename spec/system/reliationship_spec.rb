...
RSpec.describe "Relationships", type: :system do
  ...
  let!(:course2) { create(:course, user: user2) }
  let!(:course3) { create(:course, user: user3) }
  ...
  describe "フォロー中(following一覧)ページ" do
    ...
    context "ページレイアウト" do
      ...
      it "ユーザー情報が表示されていること" do
        ...
        expect(page).to have_content "コース#{user.courses.count}件"
    ...
    context "ページレイアウト" do
      ...
      describe "フィード" do
        ...
        it "フィードに自分の投稿が含まれていること" do
          expect(user.feed).to include course
        end

        it "フィードにフォロー中ユーザーの投稿が含まれていること" do
          expect(user.feed).to include course2
        end

        it "フィードにフォローしていないユーザーの投稿が含まれていないこと" do
          expect(user.feed).not_to include course3
        end
      end
    end
  end
